...
(function (globalThis, factory) {
	...
	// Browser
		...
		globalThis.ClientBootstrapWindow = factory();
	}
}(this, function () {
	...
	async function load(modulePaths, resultCallback, options) {
		...
		let showDevtoolsOnError = !!safeProcess.env['CLIENT_DEV'];
		...
		performance.mark('client/willWaitForWindowConfig');
		// /** @type {ISandboxConfiguration} */
		const configuration = await preloadGlobals.context.resolveConfiguration();
		console.log('configuration', configuration)
		performance.mark('client/didWaitForWindowConfig');
		...
		showDevtoolsOnError = safeProcess.env['CLIENT_DEV'] && !forceDisableShowDevtoolsOnError;
		const enableDeveloperKeybindings = safeProcess.env['CLIENT_DEV'] || forceEnableDeveloperKeybindings;
		...
		// Enable ASAR support
		// globalThis.ClientBootstrap.enableASARSupport(configuration.appRoot, true);

		// Get the nls configuration into the process.env as early as possible
		// const nlsConfig = globalThis.ClientBootstrap.setupNLS();
		...
		window['ClientEnvironment'] = {};

		const loaderConfig = {
			baseUrl: useCustomProtocol ?
				`${bootstrapLib.fileUriFromPath(configuration.appRoot, { isWindows: safeProcess.platform === 'win32', scheme: 'client-file', fallbackAuthority: 'client-app' })}` :
				`${bootstrapLib.fileUriFromPath(configuration.appRoot, { isWindows: safeProcess.platform === 'win32' })}`,
			// 'vs/nls': nlsConfig,
		...
		if (safeProcess.sandboxed) {
			...
			loaderConfig.amdModulesPattern = /^client\//;
		}
	...
	function registerDeveloperKeybindings(disallowReloadKeybinding) {
		...
		let listener = function (e) {
			...
			if (key === TOGGLE_DEV_TOOLS_KB || key === TOGGLE_DEV_TOOLS_KB_ALT) {
				ipcRenderer.send('client:toggleDevTools');
			} else if (key === RELOAD_KB && !disallowReloadKeybinding) {
				ipcRenderer.send('client:reloadWindow');
			}
		};
